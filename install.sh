...
# systemd /etc/default/x-ui (see x-ui.service*: EnvironmentFile=-/etc/default/x-ui)
ensure_x_ui_api_env_defaults() {
    ...
    if ! grep -qs '^[[:space:]]*XUI_API_SECRET=' "$env_file" 2>/dev/null; then
        ...
        if [[ -z "${api_secret}" ]]; then
            api_secret="$(gen_random_string 64)"
        fi
        printf '%s\n' "XUI_API_SECRET=${api_secret}" >> "$env_file"
        echo -e "${green}Wrote XUI_API_SECRET to ${env_file} — use: Authorization: Bearer <secret>${plain}"
    fi
}
...
install_x-ui() {
    ...
    if [[ $release == "alpine" ]]; then
        if [ -f "x-ui.rc" ]; then
            echo -e "${green}Found x-ui.rc in extracted files, installing...${plain}"
            cp -f x-ui.rc /etc/init.d/x-ui > /dev/null 2>&1
        else
