...
// IsAPIOnly when true skips the bundled panel UI (SPA, WebSocket listener, login routes) while keeping
// the REST API under panel/api — see web.Server.initRouter and web/controller/APIController.checkAPIAuth.
func IsAPIOnly() bool {
	return strings.EqualFold(strings.TrimSpace(os.Getenv("XUI_API_ONLY")), "true")
}

// GetAPISecret returns a static shared secret loaded from XUI_API_SECRET. Callers authenticate with:
// Authorization: Bearer <same value>.
func GetAPISecret() string {
	return strings.TrimSpace(os.Getenv("XUI_API_SECRET"))
}
